...
export function Report(props): JSX.Element {
    ...
    Object.keys(structuredExpenses).forEach((kYear) => {
        ...
        Object.keys(year).forEach((kMonth) => {
            ...
            Object.keys(month).forEach((kDay) => {
                ...
                day.forEach((line: Line, index) => {
                    ...
                    if (expense.unit == `$`) {
                        expense.unit = `€`;
                        expense.value = 0.92 * (expense.value as number);
                    ...
                    const text = (
                        <>
                            {(expense.value as number).toFixed(2)}
                            {expense.unit}{" "}
                            <span style={{ opacity: 0.2 }}>{line.text}</span>
                        </>
                    );
                    dayElements.push(
                        <div key={`expense-${index}`} class={styles.expense}>
                            {text}
                        </div>
                    );
                    if (expense.unit == "$") {
                        expense.value = (expense.value as number) * 0.92;
                    }
                ...
                const dayHeader = (
                    <>
                        {kDay.toString().padStart(2, "0")}{" "}
                        <span class={styles.dayDetails}>
                            {dayTotalAmount.toFixed(2)}€
                        </span>{" "}
                        <span class={styles.dayDetailsFaded}>
                            {dayElementCount} items
                        </span>
                    </>
                );
            ...
            yearElements.push(
                <Collapsible
                    header={
                        <div key={`month-${kMonth}`} class={styles.month}>
                            {monthHeader}
                        </div>
                    }
        ...
        const yearHeader = (
            <>
                {kYear}{" "}
                <span class={styles.dayDetails}>
                    {yearTotalAmount.toFixed(2)}€
                </span>{" "}
                <span class={styles.dayDetailsFaded}>
                    {yearExpenseCount} items
                </span>
            </>
        );
        ...
        const yearElement = (
            <Collapsible
                header={
                    <div key={`report-${kYear}`} class={styles.yearTitle}>
                        {yearHeader}
                    </div>
                }
        ...
        elements.push(
            <div key={`year-${kYear}`} class={styles.year}>
                {yearElement}
            </div>
        );
    });
    ...
    return (
        <>
            <div class={styles.report}>{elements}</div>
        </>
    );
}
