...
export function getLineExpense(line: Line): LineItem {
    const items = line.items.filter((i) => i.type == `MONEY`);
    if (items.length) {
        return items.pop();
    }
}
...
export function getLineYear(line: Line): LineItem {
    const items = line.items.filter((i) => i.type == `YEAR`);
    if (items.length) {
        return items.pop();
    }
}
...
export function getLineMonth(line: Line): LineItem {
    const items = line.items.filter((i) => i.type == `MONTH`);
    if (items.length) {
        return items.pop();
    }
}
...
export function getLineDay(line: Line): LineItem {
    const items = line.items.filter((i) => i.type == `DAY`);
    if (items.length) {
        return items.pop();
    }
}
...
export function getLineDate(line, format = `yyyy.mm.dd`) {
    ...
    return `${line.date.year}.${line.date.month
        .toString()
        .padStart(2, `0`)}.${line.date.day.toString().padStart(2, `0`)}`;
}
...
export function getLineSummary(line: Line): string {
    switch (line.type) {
        ...
        case `EXPENSE`: {
            ...
            if (i.unit == "$") {
                i.value = (i.value as number) * 0.92;
                i.unit = "€";
            }
...
export function parseNote(text): Line[] {
    ...
    let lines: Line[] = text
        ...
        .map((s: string) => {
            ...
            const month = s.match(
                new RegExp(`^(${months.map((m) => `${m}`).join("|")})$`, "gi")
            );
            if (month) {
                const item: LineItem = {};
                item.type = `MONTH`;
            ...
            // $ BAT
            const bat = s.match(new RegExp(/\S+BAT/g));
            bat &&
                bat.forEach((match) => {
                    const item: LineItem = {};
                    item.type = `MONEY`;
